...
var hash = satoshiCommon.crypto.Hash.sha256(value);
var bn = satoshiCommon.crypto.BN.fromBuffer(hash);

var address = new bch.PrivateKey(bn).toAddress();
```
...
var address = new bch.PrivateKey(wif).toAddress();
```
...
var privateKey = new bch.PrivateKey('L23PpjkBQqpAF4vbMHNfTZAb3KFPBSawQ7KinFTzz7dxq6TZX8UA');
...
var address = new bch.Address(publicKeys, requiredSignatures);
```
...
var publicKeys = privateKeys.map(bch.PublicKey);
var address = new bch.Address(publicKeys, 2); // 2 of 2
