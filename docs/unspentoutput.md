# UnspentOutput
`bch.Transaction.UnspentOutput` is a class with stateless instances that provides information about an unspent output:
...
- `amount`: expected value in BTG. If the `satoshis` alias is used, make sure to use satoshis instead of BTG.
