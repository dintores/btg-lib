...
var Input = btgLib.Transaction.Input;
var Networks = btgLib.Networks;
var Opcode = btgLib.Opcode;
var Output = btgLib.Transaction.Output;
...
describe('Transaction', function() {
  ...
  it('toObject/fromObject with p2sh signatures and custom fee', function() {
    var tx = new Transaction()
      .from(p2shUtxoWith1BTG, [p2shPublicKey1, p2shPublicKey2, p2shPublicKey3], 2)
      .to([{
        address: toAddress,
        satoshis: 50000
      }])
  ...
  it('fromObject with pay-to-public-key previous outputs', function() {
    var tx = btgLib.Transaction({
      ...
      inputs: [{
        ...
        scriptString: '710x3044022024dbcf41ccd4f3fe325bebb7a87d0bf359eefa03826482008e0fe7795586ad440220676f5f211ebbc311cfa631f14a8223a343cbadc6fa97d6d17f8d2531308b533201',
        output: {
          satoshis: 5000000000,
    ...
    tx.inputs[0].should.be.instanceof(btgLib.Transaction.Input.PublicKey);
  ...
  describe('checked serialize', function() {
    ...
    it('checks output amount before fee errors', function() {
      var transaction = new Transaction();
      transaction.from(simpleUtxoWith1BTG);
      transaction
    ...
    describe('skipping checks', function() {
      ...
      it('can skip the check that avoids spending more bitcoin gold than the inputs for a transaction', buildSkipTest(
        function(transaction) {
          return transaction
            .to(toAddress, 10000000000000)
            .change(changeAddress)
  ...
  describe('checks on adding inputs', function() {
    ...
    it('will add an empty script if not supplied', function() {
      ...
      transaction.inputs[0].output.script.should.be.instanceof(btgLib.Script);
  ...
  describe('removeInput and removeOutput', function() {
    it('can remove an input by index', function() {
      ...
      transaction.inputAmount.should.equal(simpleUtxoWith1BTG.satoshis);
    ...
    it('can remove an input by transaction id', function() {
      ...
      transaction.inputs.length.should.equal(1);
      transaction.inputAmount.should.equal(simpleUtxoWith1BTG.satoshis);
      transaction.removeInput(simpleUtxoWith1BTG.txId, simpleUtxoWith1BTG.outputIndex);
      transaction.inputs.length.should.equal(0);
      transaction.inputAmount.should.equal(0);
    });
